...
SpawnPoints::SpawnPoints(const ecs::EntityId& entityId, ecs::ComponentManager* componentManager)
    : GameObject<SpawnPoints>(entityId, componentManager)
{
    this->tankTypeSpawnComponents.emplace_back(
        ...
        std::vector<SpawnPositionComponent*>{ AddComponent<SpawnPositionComponent>(Vector3i{ 0, 0, 0 }) });
}
...
void SpawnPoints::SetModel(const SpawnPointsModel& model)
{
    ...
    for (auto& pair : model.spawnPoints)
    {
        ...
        {
            spawnPositionVector.emplace_back(AddComponent<SpawnPositionComponent>(spawnPosition));
        }
        this->tankTypeSpawnComponents.emplace_back(AddComponent<TankTypeComponent>(pair.first), spawnPositionVector);
    }
}
