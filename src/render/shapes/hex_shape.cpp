...
ShapeID HexShape::GetShapeID() const
{
    ...
    seed ^= (std::size_t)(this->GetRenderingMode() + 0x9e3779b9 + (seed << 6) +
                          (seed >> 2));
...
bool HexShape::Initialize()
{
    ...
    for (auto it = vertexesSet.begin(); it != vertexesSet.end();)
    {
        ...
        this->vertexPositionData.push_back(this->GetRenderingMode() == GL_LINES
                                               ? zCoordLines
                                               : zCoordTriangles);
    }
    ...
    for (const auto& vertexes : vectorOfVertexes)
    {
        ...
        this->GetRenderingMode() == GL_TRIANGLES ? GetTrianglesIndexes(indexes)
...
std::vector<std::size_t> HexShape::GetVertexesIndexes(
    const std::vector<Vector3f>& vertexes, const std::vector<Vector3f>& vector)
{
    ...
    {
        auto vertex =
            std::find_if(vector.begin(),
                         vector.end(),
                         [&](const Vector3f& vertex) { return vertex == v; });
