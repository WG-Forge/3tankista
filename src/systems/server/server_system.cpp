...
void ServerSystem::OnSendActionEvent(const SendActionEvent* event)
{
    ...
    LogInfo("Request to server: ", event->action, event->data.c_str());
    ...
    LogInfo("Response from server: ", result, response.c_str());
    if (result != Result::OKEY)
    {
        LogWarning("Result status is not OKEY ", static_cast<int>(result));
    }
