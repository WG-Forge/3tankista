...
double Matrix3d::det() const
{
    ...
    double det =
        self(0, 0) *
            Matrix2d({ { self(1, 1), self(1, 2) }, { self(2, 1), self(2, 2) } })
                .det() -
        self(0, 1) *
            Matrix2d({ { self(1, 0), self(1, 2) }, { self(2, 0), self(2, 2) } })
                .det() +
        self(0, 2) *
            Matrix2d({ { self(1, 0), self(1, 1) }, { self(2, 0), self(2, 1) } })
                .det();
    return det;
}
...
{
    json = nlohmann::json{ { "x", vector3i.x() },
                           { "y", vector3i.y() },
                           { "z", vector3i.z() } };
}
